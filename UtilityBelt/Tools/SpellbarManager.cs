...
namespace UtilityBelt.Tools {
    [Name("SpellbarManager")]
    public class SpellbarManager : ToolBase {
        internal HudCombo SpellbarManagerUIProfilesCombo;
        private HudButton SpellbarManagerUIProfileCopyTo;
        private HudButton SpellbarManagerUIProfileReset;
        private HudButton SpellbarManagerUIProfileImport;

        /// <summary>
        /// The default character SpellbarManager ui path
        /// </summary>
        public string CharacterSpellbarManagerUIFile { get => Path.Combine(Util.GetCharacterDirectory(), SpellbarManagerUIProfileExtension); }

        /// <summary>
        /// The file path to the currently loaded settings profile
        /// </summary>
        public string SpellbarManagerUIProfilePath {
            get {
                if (UIProfile == "[character]")
                    return CharacterSpellbarManagerUIFile;
                else
                    return Path.Combine(Util.GetProfilesDirectory(), $"{UIProfile}.{SpellbarManagerUIProfileExtension}");
            }
        }

        public static readonly string SpellbarManagerUIProfileExtension = "SpellbarManagerui.json";

        #region Config
        [Summary("SpellbarManager UI profile. Choose [character] to use a private copy of settings for this character.")]
        ...
        public class SpellbarManagerUISettings : ISetting {
        ...
        public readonly SpellbarManagerUISettings SpellbarManagerUI = new SpellbarManagerUISettings();
        #endregion

        #region Commands
        #region /ub getui
        [Summary("Returns the SpellbarManager window positions, encoded as a base64 string")]
        [Usage("/ub getui")]
        ...
        [Summary("Sets the SpellbarManager window positions, based on a base64 string")]
        ...
        [Summary("Set SpellbarManager resolution. This will take effect immediately, but will not change the settings page, or persist through relogging.")]
        [Usage("/ub resolution <width> <height>")]
        [Example("/ub resolution 640 600", "Set SpellbarManager resolution to 640 x 600")]
        ...
        [Summary("Sets SpellbarManager texture options. This will take effect immediately, but will not change the settings page, or persist through relogging.")]
        ...
        [Summary("limits frames while SpellbarManager is not activated")]
        ...
        #region /ub quit
        [Summary("Closes the SpellbarManager")]
        [Usage("/ub quit")]
        [CommandPattern("quit", @"^$")]
        public void Quit(string _, Match _2) {
            Logger.WriteToChat($"Quitting SpellbarManager");
            PostMessage(UB.Core.Decal.Hwnd, 0x0002 /* WM_DESTROY */, (IntPtr)0, (UIntPtr)0);
        }
        #endregion
        #endregion Commands

        public SpellbarManager(UtilityBeltPlugin ub, string name) : base(ub, name) {

        }

        public override void Init() {
            base.Init();
            SpellbarManagerUIProfilesCombo = (HudCombo)UB.MainView.view["SpellbarManagerUIProfilesCombo"];
            SpellbarManagerUIProfileCopyTo = (HudButton)UB.MainView.view["SpellbarManagerUIProfileCopyTo"];
            SpellbarManagerUIProfileReset = (HudButton)UB.MainView.view["SpellbarManagerUIProfileReset"];
            SpellbarManagerUIProfileImport = (HudButton)UB.MainView.view["SpellbarManagerUIProfileImport"];

            SpellbarManagerUIProfilesCombo.Change += SpellbarManagerUIProfilesCombo_Change;
            SpellbarManagerUIProfileCopyTo.Hit += SpellbarManagerUIProfileCopyTo_Hit;
            SpellbarManagerUIProfileReset.Hit += SpellbarManagerUIProfileReset_Hit;
            SpellbarManagerUIProfileImport.Hit += SpellbarManagerUIProfileImport_Hit;

            UIProfile.Changed += SelectedSpellbarManagerUIProfile_Changed;

            //UB.SpellbarManagerUISettings.Changed += SpellbarManagerUI_Changed;

            UB.MainView.PopulateProfiles(SpellbarManagerUIProfileExtension, SpellbarManagerUIProfilesCombo, UIProfile);
            RestoreSpellbarManagerUI();
        }

        private void SelectedSpellbarManagerUIProfile_Changed(object sender, SettingChangedEventArgs e) {
            //UB.SpellbarManagerUISettings.SettingsPath = SpellbarManagerUIProfilePath;
            UB.MainView.PopulateProfiles(SpellbarManagerUIProfileExtension, SpellbarManagerUIProfilesCombo, UIProfile);
        }

        private void SpellbarManagerUI_Changed(object sender, SettingChangedEventArgs e) {
            if (e.Setting.Parent is UIElementVector v) {
                RestoreSpellbarManagerUIElement(v);
            }
        }

        private void RestoreSpellbarManagerUI() {
            foreach (var element in SpellbarManagerUI.GetChildren()) {
                if (element is UIElementVector vector) {
                    RestoreSpellbarManagerUIElement(vector);
                }
            }
        }

        private void RestoreSpellbarManagerUIElement(UIElementVector vector) {
            if (!vector.IsDefault) {
        ...
        private void SpellbarManagerUIProfileReset_Hit(object sender, EventArgs e) {
            //UB.MainView.ResetProfile(SettingType.SpellbarManagerUI, UB.SpellbarManagerUISettings, UIProfile);
        }

        private void SpellbarManagerUIProfileCopyTo_Hit(object sender, EventArgs e) {
            UB.MainView.CopyProfile(UIProfile, SpellbarManagerUIProfilePath, (v) => {
                return Path.Combine(Util.GetProfilesDirectory(), $"{v}.{SpellbarManagerUIProfileExtension}");
            });
        }

        private void SpellbarManagerUIProfilesCombo_Change(object sender, EventArgs e) {
            UIProfile.Value = ((HudStaticText)SpellbarManagerUIProfilesCombo[SpellbarManagerUIProfilesCombo.Current]).Text;
        }

        private void SpellbarManagerUIProfileImport_Hit(object sender, EventArgs e) {
            UB.MainView.ImportProfile(() => {
                foreach (int i in Enum.GetValues(typeof(UBHelper.UIElement))) {
                    System.Drawing.Rectangle t = UBHelper.Core.GetElementPosition((UBHelper.UIElement)i);
                    var field = SpellbarManagerUI.GetType().GetField(((UBHelper.UIElement)i).ToString(), Settings.BindingFlags);
                    if (field != null) {
                        ((ISetting)field.FieldType.GetField("X", Settings.BindingFlags).GetValue(field.GetValue(SpellbarManagerUI))).SetValue(t.X);
                        ((ISetting)field.FieldType.GetField("Y", Settings.BindingFlags).GetValue(field.GetValue(SpellbarManagerUI))).SetValue(t.Y);
                        ((ISetting)field.FieldType.GetField("Width", Settings.BindingFlags).GetValue(field.GetValue(SpellbarManagerUI))).SetValue(t.Width);
                        ((ISetting)field.FieldType.GetField("Height", Settings.BindingFlags).GetValue(field.GetValue(SpellbarManagerUI))).SetValue(t.Height);
                    }
                }
                Logger.WriteToChat($"Imported current SpellbarManagerUI into profile '{UIProfile}'");
                return true;
            });
        }
