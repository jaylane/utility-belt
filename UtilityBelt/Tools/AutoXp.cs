...
//        public readonly Setting<bool> EnableXpChange = new Setting<bool>(true);
        ...
        public readonly Setting<Dictionary<XpTarget, double>> Policy = new Setting<Dictionary<XpTarget, double>>(
            new Dictionary<XpTarget, double>() {
        ...
        private Dictionary<XpTarget, List<int>> GetPlan(long expToSpend, int maxSteps = 9999) {
            ...
            foreach (var t in Policy.Value) {
                //Skip invalid weights
                if (t.Value <= 0)
                    continue;

                try {
                    var cost = t.Key.CostToLevel(StopBeforeMax) ?? -1;
                    ...
                    weightedCosts.Add(t.Key, cost / Policy.Value[t.Key]);
                }
                catch (Exception e) {
        ...
        private void PrintPolicy() {
            ...
            foreach (var kvp in Policy.Value) {
                //Ignore unused in the policy
                if(kvp.Value > 0)
                Logger.WriteToChat(kvp.Key + ": " + kvp.Value);
            }
        }
    ...
    public static class XpHelper {
        ...
        public static int? CostToLevel(this XpTarget target, int haltBeforeMax = 0, int offset = 0) {
            ...
            switch (target.TargetType()) {
                ...
                case ExpTargetType.Attribute:
                    if (ATTRIBUTE_COSTS.Length > targetLevel + haltBeforeMax)
                        return ATTRIBUTE_COSTS[targetLevel];
                    break;
                case ExpTargetType.Vital:
                    if (VITAL_COSTS.Length > targetLevel + haltBeforeMax)
                        return VITAL_COSTS[targetLevel];
                    break;
                case ExpTargetType.Trained:
                    if (TRAINED_COSTS.Length > targetLevel + haltBeforeMax)
                        return TRAINED_COSTS[targetLevel];
                    break;
                case ExpTargetType.Specialized:
                    if (SPECIALIZED_COSTS.Length > targetLevel + haltBeforeMax)
        ...
        public static int GetTimesLeveled(this XpTarget target) {
            switch (target.TargetType()) {
                case ExpTargetType.Attribute:
                    return CoreManager.Current.Actions.AttributeClicks[target.AsAttribute()];
                case ExpTargetType.Vital:
                    return CoreManager.Current.Actions.VitalClicks[target.AsVital()];
                case ExpTargetType.Trained:
                case ExpTargetType.Specialized:
                    return CoreManager.Current.Actions.SkillClicks[(SkillType)(target.AsSkill() + 50)];
            }

            return -1;
        }

        public static AttributeType AsAttribute(this XpTarget target) {
            return (AttributeType)(target - ATTRIBUTE_OFFSET);
        }
        public static VitalType AsVital(this XpTarget target) {
            return (VitalType)(target - VITAL_OFFSET);
        }
        public static SkillType AsSkill(this XpTarget target) {
            return (SkillType)(target);
        }

        public static void AddExp(this XpTarget target, int exp) {
            switch (target.TargetType()) {
                case ExpTargetType.Attribute:
                    CoreManager.Current.Actions.AddAttributeExperience(target.AsAttribute(), exp);
                    break;
                case ExpTargetType.Vital:
                    CoreManager.Current.Actions.AddVitalExperience(target.AsVital(), exp);
                    break;
                case ExpTargetType.Specialized:
                case ExpTargetType.Trained:
                    CoreManager.Current.Actions.AddSkillExperience(target.AsSkill(), exp);
                    break;
                default:
                    break;
            }
        ...
        public static void IncreaseLevel(this XpTarget target) {
            var cost = target.CostToLevel() ?? 0;
            target.AddExp(cost);
        }

        public static string GetName(this XpTarget target) {
            return Enum.GetName(typeof(XpTarget), target);
        }

        public static ExpTargetType TargetType(this XpTarget target) {
            var enumVal = (int)target;

            if (enumVal >= XpHelper.VITAL_OFFSET) {
                return ExpTargetType.Vital;
            }
            if (enumVal >= XpHelper.ATTRIBUTE_OFFSET) {
                return ExpTargetType.Attribute;
            }

            //Determine skill train status
            //Have to convert to BaseSkill in the way I do.  Also needed to include the missing Summoning skill
            switch (CoreManager.Current.Actions.SkillTrainLevel[(SkillType)(target.AsSkill() + 50)]) {
                case 3:
            ...
            //CoreManager.Current.CharacterFilter.Skills[(CharFilterSkillType)target].Training;  -- Throws errors with summoning

            //Logger.WriteToChat($"Unable to find exp target type for {target.GetName()}");
    ...
    //Merged enum of all things experience may be spent on
    public enum XpTarget {
        //Attributes
        //CurrentStrength = 1,
        //CurrentEndurance = 2,
        //CurrentQuickness = 3,
        ...
        //BaseFocus = 11,
        //BaseSelf = 12,
        Strength = 1 + XpHelper.ATTRIBUTE_OFFSET,
        Endurance = 2 + XpHelper.ATTRIBUTE_OFFSET,
        Quickness = 3 + XpHelper.ATTRIBUTE_OFFSET,
        Coordination = 4 + XpHelper.ATTRIBUTE_OFFSET,
        Focus = 5 + XpHelper.ATTRIBUTE_OFFSET,
        Self = 6 + XpHelper.ATTRIBUTE_OFFSET,
        ...
        //BaseStamina = 8,
        //BaseMana = 9,
        Health = 1 + XpHelper.VITAL_OFFSET,
        Stamina = 3 + XpHelper.VITAL_OFFSET,
        Mana = 5 + XpHelper.VITAL_OFFSET,
