...
namespace ImGuiNET
{
    public static unsafe partial class ImGui {
        ...
        public static bool TextureButton(string id, ManagedTexture texture, Vector2 size, int framePadding, Vector4 bgColor) {
            PushID(id);
            PushStyleColor(ImGuiCol.Button, bgColor); // button bg
            var ret = ImageButton(texture.TexturePtr, size, v2_0, v2_1, framePadding, bgColor, *GetStyleColorVec4(ImGuiCol.Text));
            PopStyleColor(); // button bg
            PopID(); // id
        ...
        public static bool TextureButton(string id, ManagedTexture texture, Vector2 size, int framePadding, Vector4 bgColor, Vector4 tint) {
            PushID(id);
            PushStyleColor(ImGuiCol.Button, bgColor); // button bg
            var ret = ImageButton(texture.TexturePtr, size, v2_0, v2_1, framePadding, bgColor, tint);
            PopStyleColor(); // button bg
            PopID(); // id
            ...
            IntPtr user_data)
        {
            ...
            CppSharp.Runtime.MarshalUtil.memcpy((IntPtr)originalUtf8InputBytes, (IntPtr)utf8InputBytes, (uint)inputBufSize);
